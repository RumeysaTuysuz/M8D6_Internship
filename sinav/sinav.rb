...
class Student
  ...
  def self.destroy(students, student_number)
    students.reject! { |s| s.number == student_number }
  end
...
def add_student(students)
  ...
  midterm = get_valid_grade("Midterm")
  final = get_valid_grade("Final")
  
  students << Student.create(name, number, midterm, final)
  puts "Student added successfully."
end

def get_valid_grade(type)
  loop do
    print "Enter #{type} Grade (0-100): "
    grade = gets.chomp.to_f
    return grade if grade.between?(0, 100)
...
def delete_student(students)
  ...
  if students.any? { |s| s.number == number }
    Student.destroy(students, number)
...
def update_student(students)
  ...
  if student
    midterm = get_valid_grade("New Midterm")
    final = get_valid_grade("New Final")
